      ...
      <DetailsDialog
        ...
        children={
          <>
            ...
            <ol>
              {intersectingLists &&
                Object.keys(intersectingLists).map(item => {
                  return (
                    <li>
                      <Text>
                        {item}: {intersectingLists[item].join(',')}
                      </Text>
                    </li>
                  );
                })}
      ...
      <Section $fullWidth>
        ...
        <SectionCard>
          ...
          <div className="flex flex-row">
            <Text bold>
              Total summed {predicate}: {totalCount} ( missing {predicate}:{' '}
              {missingIds.length}, {totalOverlaps} overlaps in{' '}
              {Object.keys(intersectingLists).length} buckets)
            </Text>
            <div className="flex space-x-4" />
            <DetailsOpenLink
              title="details"
              description={'Click to se query / overlap details'}
              onClick={() => {
