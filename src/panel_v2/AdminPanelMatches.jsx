...
function StatsCard({ scoringFunction }) {
  ...
  const { data, error, mutate, isLoading } = useSWR(
    `/api/admin/quick_matching_statistics/?scoring_function=${scoringFunction}`,
    fetcher,
    ...
    <div className="stats shadow">
      <div className="stat">
        <div className="stat-title">
          {isLoading ? 'Loading...' : data?.scoring_function}
        </div>
        <div className="stat-value">{processedValue}</div>
        <div className="stat-desc">
          {isLoading ? 'Loading...' : data?.score_type}
        </div>
      </div>
    </div>
  );
        ...
        <div className="w-1/2 max-h-full overflow-y-scroll border-2 rounded-xl bg-base-200">
          <div className="flex flex-row gap-10 p-3 items-center content-center justify-center">
            ...
            <h3 className="font-bold text-lg">
              You sure you want to confirm this matching suggestion?
            </h3>
            <button
...
function MatchScoreListItem({ item }) {
  return (
    <div className="w-full flex flex-row gap-10 p-3 border-2 items-center content-center justify-center">
      <div className="min-w-md">
        <div>
          {item?.from_usr?.id} to {item?.to_usr?.id}
        </div>
      </div>
      <div className="min-w-md">Score: {item?.score}</div>
      <div className="min-w-md">
...
function MatchingScoreListing({}) {
  const fetcher = (...args) => fetch(...args).then(res => res.json());
  const { data, error, mutate, isLoading } = useSWR(
    `/api/admin/top_scores/?items_per_page=100`,
    fetcher,
  );
  console.log('data', data);
  if (isLoading) return <div>Loading Best Matching Scores...</div>;
  return (
    <>
      <div className="w-full p-2 justify-center content-center items-center">
        <h1 className="text-2xl">
          Total 'matchable' scores ( contains many duplicate user matchings ):{' '}
          {data?.total_items}
        </h1>
      </div>
      <div className="w-full h-full flex flex-row justify-center content-center items-center gap-2 p-2 relative overflow-y-scroll">
        <div className="w-full h-full flex flex-col gap-2">
          ...
          {data?.results?.map((item, index) => (
            <MatchScoreListItem key={item?.id ? item?.id : index} item={item} />
          ))}
        </div>
      </div>
    </>
...
function TaskMangerItem({ task }) {
  ...
  const { data, error, isLoading } = useSWR(
    completed ? null : `/api/admin/tasks/${task?.task_id}/status/`,
    fetcher,
  ...
  return (
    <div>
      <span>
        Task {task?.task_id} ({data?.info?.progress?.combinations_processed}/
        {data?.info?.progress?.total_combinations}){' '}
      </span>
      <progress
        className="progress progress-secondary w-56"
        value={parseInt(data?.info?.progress?.combinations_processed)}
        max={parseInt(data?.info?.progress?.total_combinations)}
      ></progress>
    </div>
  );
}
  ...
  return (
    <dialog id="my_modal_2" className="modal">
      <div className="modal-box">
        <h3 className="font-bold text-lg">
          Caluclating Matching options & scores can be resource intensive
        </h3>
        ...
        <div className="flex flex-col">
          ...
          <button
            ...
            onClick={() => {
              fetcher(
                `/api/admin/burst_calulate_matching_scores/?task_count=${taskAmountRef.current.value}&delay_between_tasks=${taskDelayRef.current.value}&paralel_tasks=${paralelTaskRef.current.value}`,
              ).then(data => {
                setTrackableTasks(data);
              });
            }}
          >
            Start Burst Matching Score Calculation
          </button>
          {trackableTasks.map((task, index) => (
            <TaskMangerItem key={index} task={task} />
          ))}
        </div>
      </div>
...
function CleanupOptionsModal({}) {
  ...
  return (
    <dialog id="cleanup_modal" className="modal">
      <div className="modal-box">
...
function MatchingScoreNavigation({ tab, setTab }) {
  return (
    <div className="w-full flex flex-row justify-center content-center items-center gap-2 p-2 bg-base-100">
      <div role="tablist" className="tabs tabs-boxed">
        ...
        <a
          role="tab"
          onClick={() => {
            setTab('suggested_matches');
          }}
          className={`tab ${tab == 'suggested_matches' ? 'tab-active' : ''}`}
        >
          Matching Suggestion
        </a>
      </div>
...
function SuggestedMatches({}) {
  ...
  const { data, isLoading } = useSWR(
    '/api/admin/optimize_possible_matches/',
    fetcher,
  );
  ...
  return (
    <>
      <div className="w-full p-2 justify-center content-center items-center">
        <h1 className="text-2xl">
          Total 'matchable' combinations: {data?.total_items}
        </h1>
      </div>
      <div className="w-full h-full flex flex-row justify-center content-center items-center gap-2 p-2 relative overflow-y-scroll">
        <div className="w-full h-full flex flex-col gap-2">
...
export function AdminPanelV2_Matches(props) {
  ...
  return (
    <div className="w-screen h-screen relative bg-base-300">
