...
import UserImage from '../atoms/UserImage.tsx';
...
async function processAiRequest(model, messages, updateValue) {
  ...
  const fetchResults = async () => {
    const resultsResponse = await fetch(
      `/api/admin/tasks/${task.task_id}/status/`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
const NavBar = () => {
  return (
    <div className="h-fit w-full flex flex-row bg-base-300">
...
const MatchingSuggestionView = (
  matchingUser,
  setQuickViewUser,
  addUserToSelection,
) => {
  return (
    <div className="h-fit flex flex-col items-center content-center justify-center">
      <div
        className="flex flex-row hover:bg-info p-4 rounded-xl"
        onClick={() => {
          setQuickViewUser(matchingUser);
        }}
      >
        <UserImage
          user={matchingUser.profile}
          dimensions={{
            height: 20,
            width: 20,
          }}
        />
        <div>
          {matchingUser.profile.first_name}, {matchingUser.profile.second_name}
        </div>
      </div>
      <button
...
const Base = ({ children, tableView, _key }) => {
  return tableView ? (
    <td key={_key} className={`${baseTableStyles}`}>
      {children}
    </td>
  ) : (
    <div key={_key} className={`${baseTableStyles}`}>
      {children}
    </div>
  );
};
...
const FIELDS = {
  'profile.image': (field, user, key, tableView) => {
    ...
    return (
      <Base tableView={tableView} _key={key}>
        {imageComponent}
      </Base>
    );
  },
  'state.matching_state': (field, user, key, tableView) => {
    const className =
      user.state.matching_state === 'searching' ? 'bg-error' : 'bg-success';
  ...
  'profile.user_type': (field, user, key, tableView) => {
    const className =
      user.profile.user_type === 'volunteer' ? 'bg-success' : 'bg-error';
    const span = <div className={`badge badge-md ${className}`}>{field}</div>;
    return (
      <Base tableView={tableView} _key={key}>
        {span}
...
const EMAIL_FIELD_GETTERS = {
  retrieve: (email, field, _key, _onclick = null) => {
    return (
      <td key={_key}>
        <button className="btn btn-xs" onClick={_onclick}>
          View
        </button>
      </td>
    );
  ...
  time: (email, field, _key, _onclick = null) => {
    ...
    return (
      <td key={_key}>
        <input type="datetime-local" id="datetime" value={timeConverted} />
      </td>
    );
  },
  sucess: (email, field, _key, _onclick = null) => {
    ...
    const span = (
      <div className={`badge badge-md ${className}`}>
        {email[field] ? 'sucess' : 'failure'}
      </div>
    );
...
const EmailsTable = ({ user, emails, setEmailHTML }) => {
        ...
        <tbody>
          {emails.items.map((email, i) => {
            return (
              <tr
                key={i}
                className="p-0 hover:bg-100 hover:text-success hover:border hover:border-accent"
              >
                <th className="w-20">{i}</th>
      ...
      <dialog id="email_resend_confirm_modal" className="modal">
        <form method="dialog" className="modal-box">
          <h3 className="font-bold text-lg">
            Hello Do you really want to re-send this email?!
          </h3>
...
const MATCHING_FIELD_GETTERS = {
  ...
  matchable: (matchingScore, field, _key, _onclick = null, _extras = null) => {
    ...
    const span = (
      <div className={`badge badge-md ${className}`}>
        {matchingScore[field] ? 'Matchable!' : 'X unmatchable'}
      </div>
    );
  ...
  to_usr: (matchingScore, field, _key, _onclick = null, _extras = null) => {
    const matchesListing = MatchingSuggestionView(
      matchingScore[field],
      _extras.setQuickViewUser,
      _extras.addUserToSelection,
    );
              ...
              >
                ...
                {MATCHING_FIELDS.map((field, j) => {
                  if (field in MATCHING_FIELD_GETTERS) {
                    ...
                    return MATCHING_FIELD_GETTERS[field](
                      matching_score,
                      field,
                      j,
                      () => {},
...
const UserSmsTab = ({ user }) => {
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(`/api/matching/users/${user.id}/sms/`, fetcher);
...
const UserTasksTab = ({ user }) => {
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(`/api/matching/users/${user.id}/tasks/`, fetcher);
                  ...
                  >
                    <th className="w-20">
                      <input
                        ...
                        onChange={() => {
                          fetch(
                            `/api/matching/users/${user.id}/tasks/complete/`,
                            {
                              method: 'POST',
                              headers: {
                                'Content-Type': 'application/json',
    ...
    <div className="relative flex flex-col">
      ...
      {optionsOpen && (
        <div className="relative">
          <div
            className={`w-96 bg-info fixed z-60 p-4 rounded-xl flex flex-col gap-2`}
          >
            <h3 className="font-bold text-lg">
              Consider maximum dasy searching
            </h3>
            <input
              type="number"
              ref={daysSearchingInput}
              placeholder="Days searching"
              defaultValue={40}
              className="input input-bordered w-full max-w-xs"
            />
            <button
              ...
              onClick={() => {
                fetcher(
                  `/api/matching/users/${user.id}/request_score_update/?days_searching=${daysSearchingInput.current.value}`,
                ).then(res => {
...
const MatchingTable = ({
  ...
  const updateConfirmationModalState = newState => {
    ...
    } else window.match_confirmation_modal.close();
  ...
  const { data, error, isLoading, mutate } = useSWR(
    `/api/matching/users/${user.id}/scores/`,
    fetcher,
  );

  const makeMatchingApi = matchingType => {
    let postData =
      matchingType === 'proposal'
        ? {
            user1: user.id,
  ...
  const MatchDialogContent = (
    <>
      ...
      <div className="flex flex-col">
        ...
        <UserDetailsCard
          user={matchingSelectionState.user}
          _key={0}
          selectUserForDetails={() => {}}
          deselectUser={() => {}}
          partial={true}
      ...
      <div className="flex flex-row">
        <button
          className="btn btn-error"
          onClick={() => {
            setMatchingOverlayState({
              ...matchingOverlayState,
              visible: false,
            });
            setMatchingSelectionState({
              ...matchingSelectionState,
      ...
      {confirmationModalState.error && (
        <div className="text-error">{confirmationModalState.error}</div>
      )}
      {confirmationModalState.success && (
        <div className="text-success">{confirmationModalState.success}</div>
      )}
    </>
  );
  ...
  return (
    <>
      <div className="flex flex-grow w-full flex-col content-center justify-start items-start">
        <div className="flex flex-row w-full h-fit content-start justify-start items-start gap-2">
          ...
          <button
            ...
            className="btn btn btn-accent btn-xs"
          >
            Make Matching Proposal
          </button>
          <button
            onClick={() => {
              if (matchingOverlayState.visible) {
            ...
            className="btn btn btn-info btn-xs"
          >
            Make Matching{' '}
          </button>
          <button
            onClick={() => {
              if (matchingOverlayState.visible) {
                ...
                setMatchingOverlayState({
                  ...matchingOverlayState,
                  visible: true,
                  title: 'Get Or Caclulate Score Between',
                  type: 'score',
                });
                setMatchingSelectionState({
                  ...matchingSelectionState,
        ...
        <div className="flex flex-row w-full h-fit items-end content-end justify-end">
          <div className="tabs tabs-boxed">
            <a
              className={`tab ${
                listingSelection === 'listing' ? 'tab-active' : ''
              }`}
            >
              Score Listing
            </a>
            <a
              className={`tab ${
                listingSelection === 'selection' ? 'tab-active' : ''
              }`}
            >
              Custom selection
            </a>
      ...
      {matchingOverlayState.visible && (
        <div className="w-240 h-full bg-base-200 z-80 absolute">
          ...
          <div className="flex flex-col">
            ...
            {matchingSelectionState?.user ? (
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (matchingOverlayState.type === 'proposal')
                        title =
                          'Are you sure you want to make this MATCHING PROPOSAL?';
                      else if (matchingOverlayState.type === 'matching')
                        title = 'Are you sure you want to make this MATCH?';
                      else if (matchingOverlayState.type === 'score')
                        title =
                          'Are you sure you want to get the score between these users?';
                      updateConfirmationModalState({
                        visible: true,
                        title: title,
                        text:
                          matchingOverlayState.type === 'score'
                            ? ''
                            : "We will confirm first that the user are matchable and then make the match. If this fails you can 'force' a match if you want.",
                      });
                    }}
                  >
                    Confirm
              ...
              <div className="text-2xl bg-info rounded-xl">
                No User Selected
              </div>
            )}
          </div>
            ...
            return (
              ...
              >
                <th className="w-20">
                  <input
                    type="checkbox"
                    checked={selectedUsersHashes.indexOf(user.hash) != -1}
                    className="checkbox ml-2"
                    onChange={() => {
                      if (selectedUsersHashes.indexOf(user.hash) != -1) {
                        setSelectedUsers(
                          selectedUsersHashes.filter(
                            hash => hash !== user.hash,
                          ),
                        );
                      } else {
      ...
      <div className="w-full flex flex-row content-center items-center justify-center sticky bottom-2 z-50">
        <div className="bg-slate-100 rounded-xl flex items-center justify-center p-2 gap-2 shadow-xl">
          ...
          <div className="form-control">
            ...
            />
            <button
              className="btn btn-xs bg-sky-200"
              onClick={() => {
            ...
            {users.page !== users.first_page && (
              <>
                ...
                <button
                  ...
                  onClick={() => onClickPage(users.page - 1)}
                >
                  «
                </button>
              </>
            )}
            <button className="join-item btn bg-sky-200">
              Page {users.page}
            </button>
            {users.next_page && (
              <button
                className="join-item btn"
                onClick={() => onClickPage(users.next_page)}
              >
                »
              </button>
            )}
...
export const UserDetailsCard = ({
  ...
  let Content = <></>;
  if (!tiny) {
    const ConfirmedMatches = getTableComponentUser(
      user,
      'matches.confirmed',
      0,
      false,
    );
    const UnconfirmedMatches = getTableComponentUser(
      user,
      'matches.unconfirmed',
      0,
      false,
    ...
    Content = (
      <>
        ...
        <div className="w-full h-fit">{ConfirmedMatches}</div>
        <div className="w-full h-fit text-xs text-center">
          {'matches.unconfirmed'}
        </div>
  ...
  let End = <></>;
  if (!partial) {
    const ProposedMatches = getTableComponentUser(
      user,
      'matches.proposed',
      0,
      false,
      ...
      <>
        ...
        <div class="w-full flex flex-col content-start justify-start items-start gap-2">
          ...
          <div className="flex flex-row content-center items-start justify-start">
            ...
            {readableFormatDate(user.date_joined)})
          </div>
          <div className="flex flex-row content-center items-start justify-start">
            <b className="text-l">Email</b>: {user.email}
          </div>
          <div className="flex flex-row content-center items-start justify-start">
            <b className="text-l">Phone Number</b>: {user.profile.phone_mobile}{' '}
            (Nofify via {user.profile.notify_channel})
          </div>
          <div className="flex flex-row content-center items-start justify-start">
  ...
  return (
    <div
      ...
      } bg-base-200 h-fit items-center content-center justify-center rounded-xl p-2 gap-2 mb-1 relativ ${
        !tiny ? 'hover:border-2 hover:border-error hover:bg-base-100' : ''
      }`}
      onClick={() => {
        if (partial && !tiny) {
    ...
    >
      ...
      <div className="w-full h-fit text-xs">{UserType}</div>
      {Content}
...
const UserSelectionDrawer = ({
  ...
  return (
    <div className="w-full h-full min-h-full max-h-full flex flex-row relative">
        ...
        {selectedUsers.map((user, i) => {
          return (
            <UserDetailsCard
              ...
              deselectUser={_user => {
                setSelectedUsersByHash(
                  selectedUsersHashes.filter(hash => hash !== _user.hash),
                );
              }}
            />
          );
          ...
          elements={[
            {
              ...
              content: (
                <ul className="menu w-80 bg-base-200 text-base-content text-left">
                    ...
                    return (
                      <li key={i} className="p-2 w-full">
                        <a
                          ...
                          className={`rounded-btn flex flex-col items-start ${
                            selectedList === key ? 'bg-base-100' : ''
                          }`}
                        >
...
const AdminChatMessagesDisplay = ({ user, chatMessages, mutateMessages }) => {
  console.log('MESSAGES', chatMessages, user);
  return (
    <div className="w-full">
      {chatMessages.items.map((message, i) => {
        // TODO: little fucked-up isSelf is the opposite of what I thought?
        const isSelf = message.sender === user.hash;
          ...
          <div className={`chat ${isSelf ? 'chat-end' : 'chat-start'}`}>
            <div className="chat-image avatar">
              <div className="w-10 rounded-full">
                <UserImage
                  user={isSelf ? user.profile : chatMessages.match.profile}
                  dimensions={{
                    height: 32,
                    width: 32,
                  }}
            ...
            <div className="chat-header">
              {isSelf
                ...
                : chatMessages.match.profile.first_name}
              ...
              <div className="dropdown">
                <label
                  ...
                  className={`chat-bubble ${
                    message.read ? '' : 'border-2 border-error'
                  }`}
                >
                  {message.text}
                </label>
                ...
                >
                  ...
                  <li>
                    <a
                      onClick={() => {
                        fetch(
                          `/api/matching/users/${user.id}/message_mark_read/`,
                          {
                            method: 'POST',
                            headers: {
                              'Content-Type': 'application/json',
...
const AiOptions = ({ user, chatMessages, messageText, setMessageText }) => {
  ...
  const {
    ...
    mutate,
    isLoading,
  } = useSWR(`/api/ai/models/`, fetcher, { refreshInterval: 10000 });
  ...
  return (
    <div className="w-full h-full flex flex-col p-4 rounded-xl">
        ...
        <button
          ...
          onClick={() => {
            const promptMessages = [
              {
                role: 'system',
                content: `I am the 'Support', my name is ${
                  managementUserProfile.first_name
                } ${
                  managementUserProfile.second_name
                }\nThis is my description: ${
                  managementUserProfile.description
                }\nThis is the chat history:\n\n${convertMessagesToInlineFormat(
                  user.hash,
                  chatMessages.items,
                )}`,
              },
              {
                role: 'user',
                content:
                  'Basierend auf diensen nachrichten, generiere eine antwort dich ich senden würde.',
              },
            ];
          ...
          onClick={() => {
            const promptMessages = [
              ...
              },
              {
                role: 'user',
                content:
                  'Bitte die Rechschreibung korregieren, aber die formulierung bei behalten.',
              },
            ];
            console.log('P', promptMessages);
            processAiRequest(
      ...
      <div className="w-full h-fit flex flex-row">
        <textarea
          className="textarea leading-tight flex flex-grow textarea-primary h-80"
          value={aiResponse}
          placeholder="Bio"
        ></textarea>
...
const AdminChat = ({ user, _messages }) => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR(`/api/matching/users/${user.id}/messages/`, fetcher);
      ...
      <div className="w-full h-full flex flex-col gap-2 p-2">
        {Object.keys(messages).map((message_chat, i) => {
          return (
            <div
              key={i}
              className="w-full h-fit flex flex-row rounded-xl text-2xl p-3 gap-2 hover:bg-error"
              onClick={() => {
                setChat(messages[message_chat]);
                setChatId(message_chat);
                updateQueryParams({
                  param: 'chat',
                  value: messages[message_chat].match.profile.id,
                });
              }}
            >
              <UserDetailsCard
                user={messages[message_chat].match}
                _key={2 * i + 1}
                selectUserForDetails={() => {}}
                deselectUser={() => {}}
                partial={true}
    ...
    <>
      {panes.map((pane, i) => {
        return (
          <li key={i}>
            <a
              className={`${selectedPane === pane.id ? 'bg-base-300' : ''}`}
              onClick={() => {
                setPane(pane.id);
              }}
            >
...
const NotesPane = ({ user }) => {
  ...
  const createNote = useCallback(
    debounce(newNote => {
      // A debounced function that waits 500 ms after the last call to run
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR(`/api/admin/matching/users/${user.id}/notes/`, fetcher);

  useEffect(() => {
    if (_user_notes) setValue(_user_notes);
  }, [_user_notes]);
    ...
    <dialog id="actions_confirm_modal" className="modal">
      <form method="dialog" className="modal-box max-w-full w-fit">
        <div className="flex flex-col gap-2">
          ...
          {action in schemaStates ? (
            Object.keys(schemaStates[action]).map((property, j) => {
              return (
                <div className="flex flex-row gap-2" key={j}>
                ...
                fetch(`/api/admin/quick_actions/${action}/`, {
                  ...
                  if (res.ok) {
                    res.text().then(text => {
                      ...
                      setSchemaResponseStates({
                        ...schemaResponseStates,
                        [action]: { success: text, error: null },
                      });
                    });
                  } else {
                    res.text().then(text => {
                      ...
                      setSchemaResponseStates({
                        ...schemaResponseStates,
                        [action]: { success: null, error: text },
                      });
                    });
                  }
                });
...
const ActionsPane = ({ user }) => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR(`/api/admin/quick_actions/`, fetcher);
    ...
    if (actions) {
      ...
      Object.keys(actions).map((action, i) => {
        ...
        Object.keys(actions[action].schema.properties).map((property, j) => {
          _schemaStates[action][property] =
            actions[action].schema.properties[property].default;
          if (property === 'user_id') _schemaStates[action][property] = user.id;
                ...
                <div className="w-full h-fit flex flex-col gap-2 p-2">
                  ...
                  <ThemedForm
                    ...
                    onChange={({ formData }) => {
                      setSchemaStates({
                        ...schemaStates,
                        [action]: { ...schemaStates[action], ...formData },
                      });
                    }}
                  />
                  {action in schemaResponseStates && (
                    <>
                      {schemaResponseStates[action].success && (
                      ...
                      {schemaResponseStates[action].error && (
...
function PreMatchingAppointments({ user }) {
  const fetcher = (...args) => fetch(...args).then(res => res.json());
  const {
    data: preMatchingAppointment,
    error,
    isLoading,
  } = useSWR(
    `/api/matching/users/${user.id}/prematching_appointments/`,
    fetcher,
  );
    ...
    <div className="w-full relative flex flex-col">
      {preMatchingAppointment && (
        <div className="w-full rounded-xl bg-base-300 p-4">
          {JSON.stringify(preMatchingAppointment)}
        </div>
      )}
    </div>
  );
...
const AdvancedUserDetails = ({
  ...
  const { data, error, isLoading } = useSWR(
    `/api/matching/users/${user.id}/?messages=include`,
    fetcher,
  );
          ...
          <div className="w-1/2 flex-grow p-2">
            ...
            <ul className="steps steps-vertical">
              <li className="step step-primary">
                Register {new Date(data.date_joined).toDateString()}
              </li>
              {data.state.email_authenticated ? (
              ...
              {data.matches.confirmed.items.length > 0 ? (
                <>
                  ...
                  {data.matches.unconfirmed.items.length > 0 ? (
...
export const AdminPanel = ({
  ...
  const {
    ...
    !detailUser && user_details ? `/api/matching/users/${user_details}/` : null,
    fetcher,
  );
