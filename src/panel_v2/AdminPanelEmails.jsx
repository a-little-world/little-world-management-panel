...
export function AdminPanelV2_Emails(props) {
  ...
  return (
    <div className="w-screen overflow-scroll flex flex-col justify-center items-center content-center bg-base-100 relative">
      <div className="p-10 overflow-scroll flex justify-center content-center items-center flex-row gap-2 w-full flex-wrap bg-base-200 rounded-xl">
        ...
        <ul className="menu menu-vertical lg:menu-horizontal bg-base-200 rounded-box">
          <li>
            <a
              className={`${tab === 'templates' ? 'bg-error' : ''} text-xl`}
          ...
          <li>
            <a
              className={`${tab === 'emails' ? 'bg-error' : ''} text-xl`}
...
function EmailLogItem({ email }) {
  ...
  return (
    <div className="p-10 flex justify-center content-center items-center flex-row gap-2 w-full bg-base-300 rounded-xl p-3">
      <div className="text-center">
        <a
          href={`/matching/old-emails/${email.template}`}
          className="btn btn-link"
        >
          <h1 className="text-xl text-bold">{email.template}</h1>
        </a>
        <h2
          className={`text-xl text-bold ${
            email.sucess ? 'text-success' : 'text-error'
          }`}
        >
          {email.sucess ? 'Sent' : 'Failed'}
        </h2>
      </div>
      ...
      <a className="btn btn-primary" href={email.retrieve}>
        View
      </a>
    </div>
  );
...
function EmailLogsView() {
  ...
  const { data: emailLogs, isLoading } = useSWR(
    '/api/admin/list_emails/logs/',
    fetcher,
  );

  console.log('Email logs', emailLogs);
  return (
    <div className="p-10 flex justify-center content-center items-center flex-row gap-2 w-full flex-wrap max-w-full flex-grow">
      {emailLogs &&
...
function EmailTemplatesView() {
  const fetcher = (...args) => fetch(...args).then(res => res.json());
  const { data: emailTemplates, isLoading } = useSWR(
    '/api/admin/list_emails/templates/',
    fetcher,
  );
        ...
        : emailTemplates.map((email, index) => {
            return (
              <a
                ...
                href={`/matching/old-emails/${email.name}`}
                key={index}
              >
...
function EmailDetailsSendEmail() {
  ...
  const fetcher = (...args) => fetch(...args).then(res => res.json());
  const { data: emailParams, isLoading } = useSWR(
    `/api/admin/list_emails/${emailTemplateName}/params/`,
    fetcher,
  );
  ...
  const sendEmail = async emailFormFields => {
    ...
    const res = await fetch(
      `/api/admin/list_emails/${emailTemplateName}/send/`,
      {
        method: 'POST',
        headers: {
  ...
  const fetchRenderedEmail = async emailFormFields => {
    ...
    const data = await fetcher(
      `/api/admin/list_emails/${emailTemplateName}/render/`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  return (
    <>
        ...
        {emailParams &&
          Object.keys(emailParams).map((param, index) => {
            if (AUTO_FILLED.includes(param)) {
              return (
                <>
                  <span>auto filled field</span>
                  <input
                    type="text"
                    className="input input-primary bg-error disabled"
                    placeholder={param}
                    key={index}
                  />
                </>
            ...
            return (
              <input
                ...
                placeholder={param}
                key={index}
                defaultValue={emailParams[param].default}
                onChange={e => {
                  setEmailForm({
                    ...emailForm,
      ...
      <div className="p-4 bg-base-300 flex flex-col justify-center content-center items-start p-4 rounded-xl w-fit border-2 border-base-300 hover:border-secondary gap-2 scale-75">
        ...
        {emailSendResult && (
          <div className="flex flex-col">{JSON.stringify(emailSendResult)}</div>
        )}
      </div>
    </>
  );
...
export function AdminPanelV2_EmailDetails(props) {
  ...
  const { data: renderedEmail, isLoading } = useSWR(
    `/emails/${emailTemplateName}`,
    fetcher,
  );
  console.log('Rendered email', renderedEmail);

  return (
    <div className="w-screen flex justify-center items-center content-center bg-base-100 relative">
      <div className="w-full relative flex flex-row max-h-full">
        <div className="p-4 bg-base-300 flex flex-col justify-center content-center items-center p-4 rounded-xl w-fit border-2 border-base-300 hover:border-secondary gap-4">
          ...
          <a className="btn btn-secundary" href="/matching/old-emails/">
            Back
          </a>
        </div>
        <div className="p-4 bg-base-300 flex justify-center content-center items-center p-4 rounded-xl w-fit border-2 border-base-300 hover:border-secondary scale-75 max-h-lg">
          {renderedEmail && (
            <div dangerouslySetInnerHTML={{ __html: renderedEmail }} />
          )}
        </div>
